  ...
  return (
    <div style={sectionStyles.section}>
      <div style={sectionStyles.header}>
        ...
        <button onClick={onEdit} style={sectionStyles.editBtn}>Edit</button>
      </div>
      {children}
    </div>
  );
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div style={sectionStyles.row}>
      <span style={sectionStyles.rowLabel}>{label}</span>
      <span>{value}</span>
    </div>
  );
}
...
const sectionStyles: Record<string, React.CSSProperties> = {
  ...
  editBtn: { background: 'none', border: 'none', color: 'var(--color-primary-light)', cursor: 'pointer', fontSize: 14, textDecoration: 'underline' },
  row: { display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: 14 },
  rowLabel: { color: 'var(--color-text-light)' },
};
...
export function ReviewStep() {
  ...
  return (
    ...
    >
      <Section title="Loan type" onEdit={() => goToStep(1, '/')}>
      ...
      <Section title="Personal details" onEdit={() => goToStep(2, '/personal-details')}>
      ...
      <Section title="Address" onEdit={() => goToStep(3, '/address')}>
      ...
      <Section title="Employment & income" onEdit={() => goToStep(4, '/employment')}>
      ...
      <Section title="Loan details" onEdit={() => goToStep(5, '/loan-details')}>
